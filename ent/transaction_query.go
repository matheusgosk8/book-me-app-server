...
func (_q *TransactionQuery) QueryOrder() *OrderQuery {
...
func (_q *TransactionQuery) WithOrder(opts ...func(*OrderQuery)) *TransactionQuery {
	query := (&OrderClient{config: _q.config}).Query()
...
func (_q *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := _q.withOrder; query != nil {
		if err := _q.loadOrder(ctx, query, nodes, nil,
			func(n *Transaction, e *Order) { n.Edges.Order = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TransactionQuery) loadOrder(ctx context.Context, query *OrderQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Order)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Transaction)
	for i := range nodes {
		if nodes[i].order_transactions == nil {
			continue
		}
		fk := *nodes[i].order_transactions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
